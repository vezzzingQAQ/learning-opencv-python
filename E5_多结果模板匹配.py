...
imgOriginal=cv2.imread("image/D1.jpg")
imgOriginal=cv2.pyrDown(cv2.pyrDown(imgOriginal))
img=cv2.imread("image/D1.jpg",0)#转为灰度图
img=cv2.pyrDown(cv2.pyrDown(img))
template=cv2.imread("image/DM.jpg",0)
template=cv2.pyrDown(cv2.pyrDown(template))
...
for pt in zip(*loc[::-1]):
    ...
    cv2.rectangle(imgOriginal,pt,bottom_right,(0,255,0),1)
showPicture("matchResult",imgOriginal)
